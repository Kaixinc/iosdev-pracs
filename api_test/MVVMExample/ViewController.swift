...
class ViewController: UIViewController, ItemViewModelProtocol {
    ...
    func fetchData() {
        ...
        let urlStr = "http://jsonplaceholder.typicode.com/todos/"
        let url = URL(string: urlStr)
        guard let urlUnwrapped = url else { return }
        dataProvider.dataProviderProtocol = itemViewModel
        itemViewModel.getDataFromDataProvider(url: urlUnwrapped, provider: dataProvider)
    }
